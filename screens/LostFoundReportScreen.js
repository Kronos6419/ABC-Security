...
export default function LostFoundReportScreen({ navigation }) {
    ...
    function validate() {
        ...
        if (!itemCategory) missing.push('Item category');
        if (!itemDesc.trim()) missing.push('Item description');
    ...
    async function handleSubmit() {
        ...
        const lastSeen = [
        foundBy ? `Found by: ${foundBy}` : '',
        handedTo ? `Handed to: ${handedTo}` : '',
        ].filter(Boolean).join(' | ');

        await insertReport({
        id,
        type: 'Lost / Found Property Report',
        status,
        reporterName,
        rank,
        dateStr,
        ...
        station,
        locationMore,
        incidentType: itemCategory,    // reuse column
        injuryType: '',                // not used here
        details: itemDesc,             // item details
        offenderDesc: claimant,        // person claiming the item (if any)
        lastSeen,                      // found by / handed to
        policeCalled: false,
        policeNumber: '',
        createdAt,
        lat, lng, address,
        ...
        Alert.alert('Saved', 'Lost/Found report saved locally. Will sync on Wi-Fi.');
        navigation.goBack();
    }
    ...
    return (
        ...
        <View style={styles.headerRow}>
            <Text style={styles.formTitle}>Lost / Found Property Report</Text>
        ...
        <LabeledPicker
            ...
            items={['Security Guard','Team leader','Area Supervisor']}
        />
        ...
        <LabeledPicker label="Item category" selectedValue={itemCategory} onValueChange={setItemCategory} items={catOptions} />
        <LabeledInput label="Item description" value={itemDesc} onChangeText={setItemDesc} multiline placeholder="Describe the item..." />

        <LabeledInput
            label="Found by (name/ID)"
            value={foundBy}
